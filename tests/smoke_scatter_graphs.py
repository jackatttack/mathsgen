"""Scatter graphs: focused checks, graph rendering and a specimen.

The generator is built directly, not from the registry, so this smoke can
...
def main():
    load_engine()
    from mathsgen.scatter_graphs import ScatterGraphs
    from mathsgen.pdf import render_pdf
    generator = ScatterGraphs()
    specimen = []
    for level in (1, 2, 3, 4):
        prompts, forms, answers = set(), {}, {}
        for seed in range(SEEDS):
            ...
            if q.answer["kind"] == "choice":
                answers[q.answer["value"]] = answers.get(q.answer["value"], 0) + 1
            rejects(generator, replace(q, answer={"kind": "choice", "value": "wrong"}),
                    "a wrong answer")
        ...
        sample = specimen[-1]
        print("L{} ({} distinct / {}, forms {}, choices {}): {} -> {}".format(
            level, len(prompts), SEEDS, forms, answers,
            sample.prompt.display_text, sample.answer_display.text))

    export_root = Path(__file__).resolve().parent.parent / "exports"
    export_root.mkdir(exist_ok=True)
    directory = Path(tempfile.mkdtemp(prefix="scatter_graphs_", dir=str(export_root)))
    worksheet = SimpleNamespace(title="Scatter graphs specimen", id="scatter-graphs-specimen",
                                specification={}, questions=specimen)
    for mode in ("questions", "answers"):
        print(render_pdf(worksheet, directory / (mode + ".pdf"), mode))
    print("PASS: scatter graphs, independence, rejection, graphs and specimen.")
