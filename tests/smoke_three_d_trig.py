"""3D trigonometry: focused checks, scene widths and a specimen (gates registration)."""
...
def main():
    ...
    for level in (1, 2, 3, 4):
        ...
        for seed in range(SEEDS):
            ...
            rejects(generator, replace(q, answer={"kind": "quantity", "value": "999", "unit": "cm"}),
                    "a wrong answer")
    ...
    directory = Path(tempfile.mkdtemp(prefix="three_d_trig_", dir=str(export_root)))
    worksheet = SimpleNamespace(title="3D trigonometry specimen", id="three-d-trig-specimen",
                                specification={}, questions=specimen)
    for mode in ("questions", "answers"):
        print(render_pdf(worksheet, directory / (mode + ".pdf"), mode))
    print("PASS: 3D trigonometry.")
