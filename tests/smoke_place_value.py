"""Place value and decimals: focused checks and a specimen (gates registration)."""
...
def main():
    load_engine()
    from mathsgen.place_value import PlaceValue
    ...
    for level in (1, 2, 3, 4):
        ...
        for seed in range(SEEDS):
            ...
            rejects(generator, replace(q, answer={"kind": "decimal", "value": "999"}),
                    "a wrong answer")
    ...
    directory = Path(tempfile.mkdtemp(prefix="place_value_", dir=str(export_root)))
    worksheet = SimpleNamespace(title="Place value specimen", id="place-value-specimen",
                                specification={}, questions=specimen)
    for mode in ("questions", "answers"):
        print(render_pdf(worksheet, directory / (mode + ".pdf"), mode))
    print("PASS: place value and decimals.")
