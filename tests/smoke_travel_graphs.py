"""Travel graphs: focused checks, graph rendering and a specimen.
...
def main():
    load_engine()
    from mathsgen.travel_graphs import TravelGraphs
    from mathsgen.pdf import render_pdf
    generator = TravelGraphs()
    specimen = []
    ...
    directory = Path(tempfile.mkdtemp(prefix="travel_graphs_", dir=str(export_root)))
    worksheet = SimpleNamespace(title="Travel graphs specimen", id="travel-graphs-specimen",
                                specification={}, questions=specimen)
    for mode in ("questions", "answers"):
        print(render_pdf(worksheet, directory / (mode + ".pdf"), mode))
    print("PASS: travel graphs, independence, rejection, graphs and specimen.")
