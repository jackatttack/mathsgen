"""Column vectors and exact trig values: focused checks and a specimen.
...
def main():
    load_engine()
    from mathsgen.column_vectors import ColumnVectors
    from mathsgen.exact_trig import ExactTrig
    from mathsgen.pdf import render_pdf, styles
    body = styles()["body"]
    specimen = []
    for generator in (ColumnVectors(), ExactTrig()):
    ...
    directory = Path(tempfile.mkdtemp(prefix="vectors_trig_", dir=str(export_root)))
    worksheet = SimpleNamespace(title="Column vectors and exact trig specimen",
                                id="vectors-trig-specimen",
                                specification={}, questions=specimen)
    for mode in ("questions", "answers"):
        print(render_pdf(worksheet, directory / (mode + ".pdf"), mode))
    print("PASS: column vectors and exact trig values.")
