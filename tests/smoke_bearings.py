"""Bearings: focused checks, diagram renders at every PDF width, and a specimen.

The independent check uses no SymPy, so it runs on every seed.
"""
...
GENERATOR_ID = "geometry.bearings.bearings"
SEEDS = 40
SPECIMEN_SEEDS = (5, 6)
...
def main():
    ...
    for level in (1, 2, 3, 4):
        ...
        for seed in range(SEEDS):
            ...
            rejects(generator, replace(q, answer={"kind": "bearing", "value": "999"}),
                    "a wrong answer")
    ...
    directory = Path(tempfile.mkdtemp(prefix="bearings_specimen_", dir=str(export_root)))
    worksheet = SimpleNamespace(
        title="Bearings specimen", id="bearings-specimen",
        specification={}, questions=specimen,
    )
    for mode in ("questions", "answers"):
        print(render_pdf(worksheet, directory / (mode + ".pdf"), mode))
    print("PASS: bearings validation, independence, rejection, scene widths and specimen.")
