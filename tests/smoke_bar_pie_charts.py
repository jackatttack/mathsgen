"""Bar and pie charts: focused checks, rendering and a specimen (gates registration)."""
...
def main():
    ...
    directory = Path(tempfile.mkdtemp(prefix="bar_pie_charts_", dir=str(export_root)))
    worksheet = SimpleNamespace(title="Bar and pie charts specimen", id="bar-pie-specimen",
                                specification={}, questions=specimen)
    for mode in ("questions", "answers"):
        print(render_pdf(worksheet, directory / (mode + ".pdf"), mode))
    print("PASS: bar and pie charts.")
