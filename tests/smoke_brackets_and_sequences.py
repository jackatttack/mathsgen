"""Triple brackets and geometric sequences: focused checks and a specimen.
...
def main():
    load_engine()
    from mathsgen.triple_brackets import TripleBrackets
    from mathsgen.geometric_sequences import GeometricSequences
    from mathsgen.pdf import render_pdf, styles
    body = styles()["body"]
    specimen = []
    for generator in (TripleBrackets(), GeometricSequences()):
        for level in (1, 2, 3, 4):
            ...
            for seed in range(SEEDS):
                ...
                rejects(generator, replace(q, answer={"kind": "integer", "value": "999"}),
                        "a wrong answer")
    ...
    directory = Path(tempfile.mkdtemp(prefix="brackets_sequences_", dir=str(export_root)))
    worksheet = SimpleNamespace(title="Triple brackets and geometric sequences specimen",
                                id="brackets-sequences-specimen",
                                specification={}, questions=specimen)
    for mode in ("questions", "answers"):
        print(render_pdf(worksheet, directory / (mode + ".pdf"), mode))
    print("PASS: triple brackets and geometric sequences.")
