"""Vector geometry: focused checks, notation widths, scene widths and a specimen.

The independent check uses exact Fractions, not SymPy, so it runs on every seed.
"""
...
GENERATOR_ID = "geometry.vectors.vector_geometry"
SEEDS = 40
SPECIMEN_SEEDS = (5, 6)
...
def main():
    ...
    for level in (1, 2, 3, 4):
        ...
        for seed in range(SEEDS):
            ...
            rejects(generator, replace(q, answer={"kind": "vector", "a": "99", "b": "0"}),
                    "a wrong answer")
        ...
        print("L{} ({} distinct / {}, forms {}): {} -> {}".format(
            level, len(prompts), SEEDS, forms,
            sample.prompt.text, sample.answer_display.text))

    export_root = Path(__file__).resolve().parent.parent / "exports"
    export_root.mkdir(exist_ok=True)
    directory = Path(tempfile.mkdtemp(prefix="vector_geometry_specimen_", dir=str(export_root)))
    worksheet = SimpleNamespace(
        title="Vector geometry specimen", id="vector-geometry-specimen",
        specification={}, questions=specimen,
    )
    for mode in ("questions", "answers"):
        print(render_pdf(worksheet, directory / (mode + ".pdf"), mode))
    print("PASS: vector geometry validation, independence, rejection, widths and specimen.")
