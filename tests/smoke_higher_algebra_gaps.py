"""Iteration and circle equations: focused checks, widths and a specimen."""
...
GENERATOR_IDS = ("algebra.iteration.numerical", "algebra.graphs.circle_equations")
SEEDS = 40
SPECIMEN_SEEDS = (5, 6)
COLUMN_WIDTH = 460
    ...
    for generator_id in GENERATOR_IDS:
        ...
        for level in (1, 2, 3, 4):
            ...
            print("{} L{} ({} distinct / {}, forms {}): {} -> {}".format(
                generator_id, level, len(prompts), SEEDS, forms,
                sample.prompt.text, sample.answer_display.text))

    export_root = Path(__file__).resolve().parent.parent / "exports"
    export_root.mkdir(exist_ok=True)
    directory = Path(tempfile.mkdtemp(prefix="higher_algebra_gaps_", dir=str(export_root)))
    worksheet = SimpleNamespace(
        title="Iteration and circle equations specimen", id="higher-algebra-gaps-specimen",
        specification={}, questions=specimen,
    )
    for mode in ("questions", "answers"):
        print(render_pdf(worksheet, directory / (mode + ".pdf"), mode))
    print("PASS: iteration and circle equations, independence, rejection, widths, specimen.")
