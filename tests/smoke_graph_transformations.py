"""Graph transformations: focused checks, widths and a specimen (gates registration)."""
...
def main():
    load_engine()
    from mathsgen.graph_transformations import GraphTransformations
    from mathsgen.pdf import render_pdf, styles
    body = styles()["body"]
    generator = GraphTransformations()
    specimen = []
    for level in (1, 2, 3, 4):
        ...
        for seed in range(SEEDS):
            ...
            rejects(generator, replace(q, answer={"kind": "point", "x": "99", "y": "0"}),
                    "a wrong answer")
    ...
    directory = Path(tempfile.mkdtemp(prefix="graph_transformations_", dir=str(export_root)))
    worksheet = SimpleNamespace(title="Graph transformations specimen",
                                id="graph-transformations-specimen",
                                specification={}, questions=specimen)
    for mode in ("questions", "answers"):
        print(render_pdf(worksheet, directory / (mode + ".pdf"), mode))
    print("PASS: graph transformations.")
