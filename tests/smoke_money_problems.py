"""Money problems: focused checks, form balance and a specimen."""
...
GENERATOR_ID = "ratio.money.problems"
SEEDS = 40
...
def main():
    ...
    for level in (1, 2, 3, 4):
        ...
        for seed in range(SEEDS):
            ...
            if "value" in q.answer and q.answer["kind"] == "yes_no":
                decisions[q.answer["value"]] = decisions.get(q.answer["value"], 0) + 1
            rejects(generator, replace(q, answer={"kind": "money", "pence": "1"}), "a wrong answer")
        ...
        sample = specimen[-1]
        print("L{} ({} distinct / {}, forms {}, yes/no {}): {} -> {}".format(
            level, len(prompts), SEEDS, forms, decisions,
            sample.prompt.text, sample.answer_display.text))

    export_root = Path(__file__).resolve().parent.parent / "exports"
    export_root.mkdir(exist_ok=True)
    directory = Path(tempfile.mkdtemp(prefix="money_problems_", dir=str(export_root)))
    worksheet = SimpleNamespace(title="Money problems specimen", id="money-problems-specimen",
                                specification={}, questions=specimen)
    for mode in ("questions", "answers"):
        print(render_pdf(worksheet, directory / (mode + ".pdf"), mode))
    print("PASS: money problems, independence, rejection and specimen.")
